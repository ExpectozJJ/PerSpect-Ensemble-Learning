...
def call_script(args):
    os.system("sudo ./run_topnettree.sh {}".format(args))
...
os.chdir("..")
os.chdir("..")
np.save("X_ab_l0.npy", np.transpose(np.reshape(feature,(len(feature), 36, 48, 15)), axes=(0,2,1,3)))
...
for i in range(len(args)):
    ...
    os.chdir(foldername)
    #alpha_l0(args[i])
    feature.append(np.load("alpha_l0.npy", allow_pickle=True))
    print(np.shape(feature))
    #print(np.shape(op))
    os.chdir("..")

os.chdir("..")
np.save("X_ab_alpha_l0.npy", np.transpose(np.reshape(feature,(len(feature), 36, 120, 15)), axes=(0,2,1,3)))
"""