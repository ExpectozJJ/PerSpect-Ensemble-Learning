...
nloop = 120;
idim = 2;
file0 = fopen(strcat('./wild_binding_eig_test.txt'),'w');
file1 = fopen(strcat('./wild_binding_simp_test.txt'),'w');
for j=1:3
    for k=1:3
        Elements = {'C','N','O'};
        e1 = Elements{j}; e2 = Elements{k};
        fprintf('%s %s \n', e1, e2);
        WildName = strcat('wild_binding_',e1,'_',e2,'.pts');
        ...
        for iloop=1:nloop
           ...
           %% calculate eigenvectors
            ...
            %     if numel(mathdg)>0
                [Vec,Dmat] = eig(mathdg);
                %[Vec,Dmat,flag] = eigs(mathdg, size(mathdg,1), 'smallestreal', 'MaxIterations', 500);
                %[Vec, Dmat] = svd(mathdg);
                %display(flag)
                ndg=size(Dmat,1);
                ivct=[];
...
nloop = 120;
idim = 2;
file0 = fopen(strcat('./mut_binding_eig.txt'),'w');
file1 = fopen(strcat('./mut_binding_simp.txt'),'w');
for j=1:3
    for k=1:3
        Elements = {'C','N','O'};
        e1 = Elements{j}; e2 = Elements{k};
        fprintf('%s %s \n', e1, e2);
        WildName = strcat('mut_binding_',e1,'_',e2,'.pts');
        fileID = fopen(strcat('./', WildName), 'r');
        A = fscanf(fileID, formatSpec, sizeA);
        fclose(fileID);
        for iloop=1:nloop
