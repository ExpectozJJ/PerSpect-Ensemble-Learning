...
if not os.path.isdir("./ab_PRelu_Nout_v6/"):
    os.mkdir("./ab_PRelu_Nout_v6/")  

for j in range(start, end+1):
    print('iter_num',j)

    filename = "./ab_PRelu_Nout_v6/spect_ab_cnn_{}.log".format(j)
    f = open(filename,"w+")
    f.close() 
    ...
    for train_index, test_index in kf.split(X):
        ...
        plot_model(cnn_model, to_file='./ab_PRelu_Nout_v6/cnn_model.png', show_shapes=True, dpi=200)
        plot_model(alpha_model, to_file='./ab_PRelu_Nout_v6/alpha_model.png', show_shapes=True, dpi=200)
        ...
        for attr_idx in range(11):
            ...
            cnn_model.save("./ab_PRelu_Nout_v6/spectcnn_ab_model_{}_{}_{}.h5".format(j, ii, attr_idx))
        ...
        saved_hist.append(history)
        alpha_model.save("./ab_PRelu_Nout_v6/spectcnn_ab_model_{}_{}_11.h5".format(j, ii))
        ...
        saved_hist.append(history)
        alpha_model.save("./ab_PRelu_Nout_v6/spectcnn_ab_model_{}_{}_12.h5".format(j, ii))
        ...
        plt.legend()
        plt.savefig("./ab_PRelu_Nout_v6/spectnettree_ab_loss_{}_{}.png".format(j, ii), dpi=200)

        ii += 1 

    np.save("./ab_PRelu_Nout_v6/ab_data_{}.npy".format(j), data)
    ...
    np.save("./ab_PRelu_Nout_v6/spectcnn_ab_result_whole_{}.npy".format(j), result_whole)
    np.save("./ab_PRelu_Nout_v6/spectcnn_ab_rmse_whole_{}.npy".format(j), rmse_whole)
    np.save("./ab_PRelu_Nout_v6/spectcnn_ab_pearsonr_whole_{}.npy".format(j),pearsonr_whole)