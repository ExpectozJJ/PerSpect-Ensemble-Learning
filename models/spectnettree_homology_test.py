...
filename = "./ab_homo/spectnettree_ab.log"
f = open(filename,"w+")
f.close()
...
for j in range(iter_num):
    print('iter_num',j)
    data = np.load("./ab_homo/ab_data_{}.npy".format(j), allow_pickle=True)
    result = np.zeros((3, n_num))
    indices = []
    spectcnn_models = []
    spectnettree_models = []

    X_train, y_train, X_test, y_test, alphal1_train, alphal1_test, alphal2_train, alphal2_test, aux_train, aux_test = data[0]
    ...
    cnn_train, cnn_test = [], []
    for k in range(11):
        model_name = './ab_homo/spectcnn_ab_model_{}_{}.h5'.format(j, k)
    ...
    model_name = './ab_homo/spectcnn_ab_model_{}_{}.h5'.format(j, 11)
    spectcnn_models.append(load_model(model_name, custom_objects = {"PReLU":PReLU, 'rootmse':rootmse, 'pearson_r':pearson_r}))
    X_train, y_train, X_test, y_test, alphal1_train, alphal1_test, alphal2_train, alphal2_test, aux_train, aux_test = data[0]
    ...
    model_name = './ab_homo/spectcnn_ab_model_{}_{}.h5'.format(j, 12)
    spectcnn_models.append(load_model(model_name, custom_objects = {"PReLU":PReLU, 'rootmse':rootmse, 'pearson_r':pearson_r}))
    X_train, y_train, X_test, y_test, alphal1_train, alphal1_test, alphal2_train, alphal2_test, aux_train, aux_test = data[0]
    ...
    for jj in range(3):
        rmse = np.sqrt(mean_squared_error(y_test,result[jj]))
        pearsonr = scipy.stats.pearsonr(y_test,result[jj])
...
np.save("./ab_homo/spectnettree_ab_result_whole", result_whole)
np.save("./ab_homo/spectnettree_ab_rmse_whole", rmse_whole)
np.save("./ab_homo/spectnettree_ab_pearsonr_whole",pearsonr_whole)
...
file_out = open("./ab_homo/crossvalidation_spectnettree_ab.txt","w+")
